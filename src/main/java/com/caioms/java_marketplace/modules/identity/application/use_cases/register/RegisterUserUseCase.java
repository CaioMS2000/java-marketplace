...
public class RegisterUserUseCase {
	...
	public Either<RegisterUserError, Result> execute(Params params) {
		...
		var rolesSet = Set.of(Role.USER);
		var user = new User(params.email(), rolesSet);
		var credential = Credential.password(user, password);
		var savedUser = userRepository.save(user);
		var __savedCredential = credentialRepository.save(credential);

		return Either
		        .right(new Result(savedUser.getId(), savedUser.getEmail(), savedUser.getRoles()));
	}

	public record Params(String email, String password) {
	}

	public record Result(UUID id, String email, Set<Role> roles) {
	}
}
